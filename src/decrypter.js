(() => {
  ...
  class Decrypter {
    ...
    decrypt({input, output, encryptionKey, signatureKey, signature}) {
      ...
      .then(() => {
        ...
        input.pipe(decipher);
        input.resume();
        return this._waitUntilWritablesFinish([output, verifier]);
      })
      .then(() => {
        this._debug(signature.toString('hex'));
        this._debug(holder.signature.toString('hex'));
